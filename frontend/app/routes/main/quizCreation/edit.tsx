...
export default function EditQuiz() {
    ...
    const isQuestionPlaceholder = (question: any) =>
        !question.questionTitle.trim() &&
        question.answerOptions.every((option: any) => !option.text.trim());

    const isQuestionValid = (question: any) => {
        if (!question.questionTitle.trim()) return false;
        if (question.timeLimit === undefined || question.timeLimit < 0) return false;
    ...
    useEffect(() => {
        ...
        if (!editId) {
            setLoadError("No quiz selected for editing.");
            setIsLoading(false);
            return;
        }
        ...
        const fetchQuiz = async () => {
            ...
            try {
                const response = await apiClient(`/api/quizzes/${editId}`);
                if (!response.ok) {
                    const body = await response.json().catch(() => null);
                    throw new Error(body?.message || `Unable to load quiz (status ${response.status}).`);
                }

                const data = await response.json();
                setQuizTitle(data.title ?? "");
                setQuizImage(data.image ?? null);
                setIsPublic(data.is_public ?? true);

                const loadedQuestions = Array.isArray(data.questions) && data.questions.length > 0
                    ? data.questions.map((question: any) => {
                          ...
                          if (question.question_type === "multiple_choice") {
                              while (answerOptions.length < 4) {
                                  answerOptions.push({ text: "", correct: false });
                              }
                          }

                          if (question.question_type === "fill_in_blank" && answerOptions.length === 0) {
                              answerOptions.push({ text: "", correct: true });
                          }

                          return {
                              questionTitle: question.question_text ?? "",
                              type: question.question_type === "fill_in_blank" ? "fill_in_blank" : "multiple_choice",
    ...
    const handleSaveQuiz = () => {
        if (!quizId) {
            setSaveError("Unable to save quiz because no quiz ID was provided.");
            return;
        }

        saveQuizChangesToDatabase();
    };
    ...
    const saveQuizChangesToDatabase = async () => {
        setIsSaving(true);
        setSaveError(null);
        try {
            ...
            const questionsToSave = questions.filter((question) => !isQuestionPlaceholder(question));

            if (quizImage && quizImage.startsWith("data:")) {
                const form = new FormData();
                form.append("title", quizTitle);
                form.append("questions", JSON.stringify(questionsToSave));
                ...
                response = await apiClient(endpoint, {
                    ...
                    body: JSON.stringify({ title: quizTitle, questions: questionsToSave, image: quizImage, is_public: isPublic }),
                });
            }
    ...
    if (isLoading) {
        return (
            <div className="flex h-[80vh] items-center justify-center text-center px-4">
                <div>
                    <div className="mb-4 text-2xl font-semibold">Loading quiz...</div>
                    <p className="text-gray-500">Fetching quiz details so you can make updates.</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="flex h-[80vh] items-center justify-center text-center px-4">
                <div className="w-full max-w-2xl bg-white rounded-3xl shadow-lg border border-gray-200 p-8">
                    <h1 className="text-3xl font-bold mb-4">Unable to edit quiz</h1>
                    <p className="text-gray-600 mb-6">{loadError}</p>
                    <div className="flex justify-center gap-3">
                        <Button onClick={() => navigate('/profile/quizzes')}>Back to My Quizzes</Button>
                        <Button variant="secondary" onClick={() => navigate('/browse')}>Browse quizzes</Button>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4">
            <div className="w-full max-w-4xl p-6 bg-white rounded-3xl shadow-lg border border-gray-200">
                ...
                <p className="text-gray-600 mb-8">
                    Update the quiz title, image, question content, and settings.
                </p>
                ...
                {saveSuccess ? (
                    <div className="p-10 text-center">
                        <h2 className="text-3xl font-semibold mb-4 text-green-600">Quiz updated successfully!</h2>
                        <p className="text-gray-600 mb-8">Your changes have been saved. You can return to My Quizzes or continue editing.</p>
